## [Reduction]
**Bloat:** `Sanitizer<'a>` RAII structure with explicit `Display` trait implementation in `src/codegen.rs` which added verbosity and abstraction bloat.
**Cut:** Replaced `Sanitizer<'a>` with a single function `sanitize_to_string(name, capitalize)`.
**Saved:** 15 lines of boilerplate structs and trait implementations. Reduced cognitive load for string formatting.
## [Reduction]
