...
## [Reduction]
**Bloat:** `src/experimental/` directory containing `interpreter.rs` and `mod.rs` which were speculative "tree-walk interpreter" components, duplicating execution logic and deviating from the transpiler strategy.
**Cut:** Deleted the `src/experimental` directory completely and removed its module export from `src/lib.rs`.
**Saved:** 2 files, ~230 lines of dead code and speculation.
