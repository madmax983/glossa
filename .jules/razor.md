...
## [Reduction]
**Bloat:** `src/experimental/interpreter.rs` was an experimental tree-walk interpreter hidden behind a `nova` feature flag (Speculative Generality).
**Cut:** Deleted the entire `src/experimental` module containing the unused interpreter.
**Saved:** 1 directory, 2 files, removed YAGNI interpreter code, reduced project bloat.
