...
## Forge Refactor Strategy: Orchestration Functions in runner.rs
**Learning:** CLI tools often repeat validation and setup logic (file existence, size checks, cache management) across multiple commands. Extracting these into helper functions (`load_source`, `analyze_source`) and dedicated modules (`cache.rs`) clarifies the core intent of each command. However, `run_file` itself is an orchestrator spanning 100+ lines (God Function). We should break it down.
**Action:** Identify repeated "pre-flight" checks in command handlers and unify them into a single pipeline or context struct early. Refactor `run_file` to delegate to smaller semantic pieces (e.g. `run_cached`, `compile_and_build`, `execute_binary`).
