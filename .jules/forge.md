...
## 2026-03-30 - Refactoring classification functions
**Learning:** Large functions containing many early-return matching blocks, like `extract_value` and `classify_expression`, can be simplified significantly by extracting complex nested match/if logic into standalone `try_build_*` or `extract_*` helper functions. However, if using nested if statements, clippy might suggest combining them, which can reduce readability. Using `#[allow(clippy::collapsible_if)]` is acceptable to keep the logic separate and readable if one logic strictly builds off the option result of another.
**Action:** Always aim to extract large, self-contained conditional logic into helper functions to flatten the main flow. If extracting creates nested `if` bindings that clippy flags, either use `#[allow(clippy::collapsible_if)]` or combine the conditions if it does not sacrifice readability.
