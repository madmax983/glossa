...
**Refactoring `check_else_pattern_in_expression` and `check_conditional_start`**\n**Learning:** Constructing intermediate `Vec<String>` and calling `.join(" ")` to check known text sequences creates unnecessary heap allocations. Using an array of `&str` constants and  with `w.normalized == lexicon::ELSE_PATTERN_WORDS[i]` allows for zero-cost semantic matching.\n**Action:** Use array constants in `lexicon.rs` to compare known string patterns instead of generating joined strings. Additionally, extract `&str` slices directly from `Expr::Word` via `.as_str()` rather than calling `.clone()` on the owned `String`.

**Refactoring `check_else_pattern_in_expression` and `check_conditional_start`**
**Learning:** Constructing intermediate `Vec<String>` and calling `.join(" ")` to check known text sequences creates unnecessary heap allocations. Using an array of `&str` constants and `.iter().enumerate().all()` with `w.normalized == lexicon::ELSE_PATTERN_WORDS[i]` allows for zero-cost semantic matching.
**Action:** Use array constants in `lexicon.rs` to compare known string patterns instead of generating joined strings. Additionally, extract `&str` slices directly from `Expr::Word` via `.as_str()` rather than calling `.clone()` on the owned `String`.
