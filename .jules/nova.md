...
## 🌟 The Sibyl (ἡ Σίβυλλα)
**Concept:** A CLI tool (`glossa sibyl`) that inspects the type definitions (`εἶδος`) within a ΓΛΩΣΣΑ program and generates mock JSON data based on them.
**Fate:** Proposed
**Lesson:** The semantic structure of type definitions allows easy mock generation, bridging ancient type semantics with modern JSON tooling.
