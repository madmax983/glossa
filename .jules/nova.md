...
**Concept:** An exporter (`glossa scribe`) that transpiles Glossa structs (`εἶδος`) directly to TypeScript `interface` definitions.
**Fate:** Proposed
**Lesson:** Shows that Glossa can act as a domain-driven schema language capable of powering modern web frontend types, extending its reach beyond Rust backends and SQL databases.
