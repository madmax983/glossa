...
**Concept:** A CLI tool (`glossa scribe`) that exports the `AnalyzedProgram` AST to a structured JSON format.
**Fate:** Proposed
**Lesson:** Creating a manual recursive visitor to format JSON allows us to provide a serialization format for external integrations without introducing large dependencies like `serde` into the core compiler ecosystem, enforcing our focus on zero-cost or dependency-free abstractions where applicable.
