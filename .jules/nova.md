...
## 🌟 The Sibyl (ἡ Σίβυλλα)
**Concept:** A CLI tool (`glossa sibyl`) that generates test boilerplate (`δοκιμή`) for all functions defined in a ΓΛΩΣΣΑ program, effectively scaffolding out TDD files.
**Fate:** Merged
**Lesson:** Iterating over the semantic AST allows us to extract function parameters to generate default values as commented-out assignments, saving users significant typing.
