...
**Concept:** A CLI tool (`glossa scribe`) that transpiles Glossa structs (`εἶδος`) to TypeScript `interface` definitions.
**Fate:** Merged
**Lesson:** Exporting structures to `.d.ts` definitions reveals that Glossa types map exceptionally well to modern frontend boundaries, enabling full-stack "ancient-backend to modern-frontend" integrations via JSON APIs.
