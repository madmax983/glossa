...
**2023-10-27 - [Memory Exhaustion via Unbounded read_line in Interactive Loops]
**Threat:** The REPL and Mentor tools read from standard input in interactive loops using `read_line`, waiting for a newline character `\n`. If a malicious user pipes an infinite stream of characters lacking a newline (like `/dev/zero` or `b" ".repeat(...)`), `read_line` will allocate memory indefinitely until the application crashes with an Out-of-Memory (OOM) panic, leading to a Denial of Service.
**Defense:** Wrap the mutable input reader reference in a bounded `std::io::Take` limiter using `input.by_ref().take(LIMIT)`. This safely caps the maximum allocation per read to a predefined boundary (e.g., 50,000 bytes). Once the limit is reached, `read_line` successfully returns, the loop evaluates the chunk, handles errors gracefully, and drops the 50KB allocation before iterating again, effectively neutralizing the memory exhaustion vector.
