...
**2025-06-03 - [Test Flakiness / DoS via unhandled Command::output() Result]**
**Threat:** `Command::new(...).output().unwrap()` was used in `src/tools/runner.rs` and `src/tools/tester.rs` during testing, which could cause tests to panic and crash instead of failing gracefully when the external binary (like `glossa` or `rustc`) is not present in the environment. Similarly, `.unwrap()` was heavily used for `fs` operations which bypassed safe error handling and could cause unexplained test crashes (DoS).
**Defense:** Replaced `.unwrap()` with `.expect("...")` in `src/tools/runner.rs`, `src/tools/tester.rs`, and `tests/nova_coverage.rs`, ensuring test environments panic with a descriptive message rather than a raw unwrap failure, preventing ambiguous CI failures.
