...
**2024-05-15 - Unbounded File Read DoS in Scholar Tool**
**Threat:** The `run_scholar` tool used `std::fs::read_to_string` in its test cases to read generated Markdown files. This loads the entire file into memory without bounds checking. An attacker could theoretically supply a massive `.γλ` file that generates a massively large `.doc.md` file, leading to memory exhaustion and a Denial of Service (DoS) during the test execution.
**Defense:** Replaced the direct use of `fs::read_to_string` with bounded reads using `std::io::Read::take(1024 * 1024 + 1).read_to_string()` in `src/tools/scholar.rs` test cases to prevent out-of-memory errors.
