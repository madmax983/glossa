...
**2025-05-18 - [Unbounded Stream DoS in REPL and Mentor Read_Line]**
**Threat:** A memory exhaustion DoS vulnerability existed in `src/tools/repl.rs` and `src/tools/mentor.rs` where the `run_repl_inner` and `run_mentor_inner` functions used unbounded `input.read_line(&mut line)` calls. If an attacker or script piped an infinite stream without newlines (e.g. using `std::io::repeat(b' ')`), it would cause the memory buffer to grow indefinitely and crash the application with an Out-of-Memory (OOM) error.
**Defense:** Replaced the unbounded `read_line` calls with a constrained reader using `.take(50000)` via `std::io::Read::take(input.by_ref(), 50000)` combined with `std::io::BufReader::new`. This capped individual read operations to prevent buffer exhaustion, enabling graceful rejection or EOF-like termination when the stream exceeded limits. Added new tests `test_repl_dos_unbounded_read` and `test_mentor_dos_unbounded_read` using a simulated 100,000-byte stream to ensure stability without OOM panics.
