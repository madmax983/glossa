...
## [Untangling the Parser Knot]
**Tangle:** `src/parser/grammar.rs` housed both the raw Pest token definitions (`GlossaParser`, `Rule`) and the high-level `parse` wrapper. This forced `statements.rs` and `expressions.rs` to import `Rule` from `grammar.rs`, while `grammar.rs` imported `build_statement` from `statements.rs`, creating a circular dependency web.
**Blueprint:** Extracted the auto-generated `pest` parser (`GlossaParser` and `Rule`) into a new foundational module `src/parser/core.rs`. Updated all submodules to import `Rule` from `core`, restoring a clean tree dependency structure.
