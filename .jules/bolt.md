...
**[Optimizing String format! Allocations]**
**Learning:** Using `format!` recursively or heavily for simple concatenations results in many small intermediate heap allocations that hurt performance.
**Action:** Replaced `format!` with `std::fmt::Write` recursively writing to a pre-allocated `String::with_capacity` buffer in `tell_type`, and explicit `push_str` calls for simple string concatenation in `generate_rust_file`.
