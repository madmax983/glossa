...
## 2026-06-27 - [Removing unsupported error messages from Troubleshooting]
**Confusion:** The user reported in `ECHO_ISSUE.md` that the error messages described in the `README.md` ("Undefined Variable", "Double Subject", and "Missing Verb") do not actually trigger. Trying to force them either silently ignores the problem (evaluating to zero) or causes an Internal Compiler Error (codgen fail).
**Clarification:** I verified that the compiler's semantic mapping contains deep bugs that prevent these errors from being surfaced smoothly without breaking tests. As "Bard", I must follow the directive to never write documentation that lies. Since the features do not work, the most essential documentation fix is to remove them entirely from the `README.md` to prevent confusing users, rather than risking core regressions attempting to fix the backend bugs.
