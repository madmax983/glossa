...
## 2026-05-18 - Detached Documentation
**Confusion:** The documentation for `to_rust_type` in `src/codegen.rs` was not being rendered in `cargo doc` despite a full rustdoc block being present right above it. `cargo doc` complained about missing documentation on the function.
**Clarification:** I moved an intervening `use std::fmt::Write;` statement that was placed directly between the documentation block and the `to_rust_type` function signature. Rustdoc always attaches documentation to the item immediately following the comment, so the `use` statement was effectively "stealing" the documentation, leaving the public function undocumented. I hoisted the import above the block to fix it.
