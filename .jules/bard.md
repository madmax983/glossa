...
## 2026-05-08 - Missing Executable Examples for Tool Entrypoints
**Confusion:** Several tools within the `src/tools/` directory (e.g., `papyrus`, `alchemist`, `weave`, `cartographer`, `gnomon`, `catalog`, `repl`, `haruspex`, `mosaic`, `auditor`) had public functions representing their entry points (like `run_papyrus` and `run_auditor`), but were missing the executable `## Examples` block. The lack of examples meant that users looking at the documentation wouldn't have concrete, copy-pasteable snippets for how to use these tools programmatically. The `missing_docs` warning didn't catch this because the tool itself doesn't enforce the internal convention of having an `## Examples` section.
**Clarification:** I added `rust,no_run` doc-tests to all of these tool entry functions. These examples demonstrate standard usage (e.g., reading a file path or handling the result) while skipping actual execution during `cargo test` to prevent I/O blocking or output spamming.
