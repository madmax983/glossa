...
## 2025-10-31 - [Doc comments separation by imports]
**Confusion:** I attempted to document a function `pub fn to_rust_type` in `src/codegen.rs` with `cargo doc -W missing_docs`. However, the documentation warned that the function was missing documentation, even though the comment `/// Convert a Glossa type ...` was just above it. This was because `use std::fmt::Write;` was directly below the documentation comment, breaking the association between the doc comment and the target function.
**Clarification:** I moved the documentation string `///` directly above the `pub fn to_rust_type` definition, swapping it with the `use std::fmt::Write;` line. A doc comment binds to the first non-comment item following it. Placing an unrelated item like a `use` statement between the doc comment and the function causes the doc to attach to the `use` statement instead, leading to missing documentation warnings on the intended target.
