...
**2025-03-05 - Avoid Modifying Environment Variables in Concurrent Tests**
**Learning:** Altering environment variables (like `GLOSSA_RUSTC_CMD`) directly using `std::env::set_var` within an integration test execution modifies global process state, leading to race conditions and "flaky" parallel test suite failures. Spawning a detached child process (using `std::process::Command::new`) provides a clean sandbox for environment variable overrides but prevents local coverage tools (`cargo llvm-cov`) from tracking line executions within that detached binary natively.
**Action:** When testing failure modes triggered by environment overrides (e.g., missing compiler executable), document that related lines (like `.unwrap_err()` OS code matching) will show as uncovered. Rely on table-driven unit tests to achieve parsing coverage elsewhere rather than battling OS-level process mocking constraints in coverage gates.
