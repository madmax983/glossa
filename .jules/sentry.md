...
**[Codegen Runtime Math Overflow Panic Coverage]**
**Learning:** Found potential runtime panics generated by `src/codegen.rs` for mathematical overflow operations (like `checked_add`, `checked_sub`, `checked_mul`, `checked_div`, `checked_rem`) missing runtime code coverage. When generating Rust code, simply verifying that the `expect` string exists in the string output of the code generator is insufficient to guarantee the generated Rust application will panic correctly.
**Action:** Wrote isolated integration tests in `tests/sentry_codegen_perf.rs` for all checked math binary operators to guarantee the panic safeguards execute as intended, crash the runtime process, and generate the appropriate standard Greek error messages (like `Ὑπερχείλισις ἀριθμοῦ` or `Διαίρεσις διὰ τοῦ μηδενός`).
