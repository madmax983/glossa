...
**Learning:** In the PEG parsing stage, using `match pair.as_rule()` with a generic `_ => Err(ParseError::UnexpectedRule(...))` fallback is good defensive practice, but these branches remain permanently uncovered because the `pest` grammar guarantees input validity before it reaches the AST builder.
**Action:** Craft manual `pest` `Pairs` (often by parsing mismatched rules intentionally) and feed them to the specific AST builder functions inside an embedded `#[cfg(test)] mod tests` block to cover these critical safety guards.
**Testing AST Patterns Exclusively Built During Semantic Analysis**
**Learning:** Some specific patterns (like a Match Pattern missing a body, or unexpected elements inside loops) are validated by returning `GlossaError::semantic` deep inside `semantic/control_flow.rs`. Due to constraints on the `pest` parser, you cannot easily generate these edge cases from a raw string query. Attempting to match exact error strings directly across different modules often results in test failures, because underlying checks may intercept with varied messages (e.g. `Ἄγνωστον ὄνομα: αγνωστου` instead of `Expected word for range`).
**Action:** When manually assembling `Statement` and `Expr` nodes to simulate unparsable scenarios for testing the semantic bounds checks, inspect the resulting `unwrap_err().to_string()` directly or assert multiple potential substrings rather than assuming a single specific string variant, to ensure robustness.
