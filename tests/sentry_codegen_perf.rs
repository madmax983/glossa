...
#[test]
fn test_codegen_runtime_rem_by_zero_panic() {
    let dir = tempfile::tempdir().unwrap();
    let rs_path = dir.path().join("test_rem_by_zero.rs");
    ...
    let stmt = AnalyzedStatement::Expression(vec![rem_expr]);
    let program = AnalyzedProgram {
        statements: vec![stmt],
        scope: Scope::new(),
    };
    ...
    let exe_path = dir.path().join("test_rem_by_zero");
    let rustc_status = std::process::Command::new("rustc")
        .arg(&rs_path)
        .arg("-o")
        .arg(&exe_path)
