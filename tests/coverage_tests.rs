...
fn test_assignment_logic() {
    ...
    let _source = "ξ πέντε μετά ἔστω. ξ δέκα γίγνεται.";
...
fn test_assignment_error_immutable() {
    ...
    let _source = "ξ πέντε ἔστω. ξ δέκα γίγνεται.";
    // "immutable variable"
...
fn test_assignment_error_no_value() {
    ...
    // "ξ γίγνεται."
    let _source = "ξ πέντε μετά ἔστω. ξ γίγνεται.";
...
fn test_collection_instantiation() {
    ...
    // "s new Set let."
    let _source = "σ νέον σύνολον ἔστω.";
    compile_success(source);

    // HashMap instantiation
    // "m new Map let."
    let _source = "χ νέον χάρτης ἔστω.";
    compile_success(source);
}
...
fn test_binding_with_false_participle() {
    ...
    let _source = "τοπικον πέντε ἔστω. τοπικον λέγε.";
    compile_success(source);
}
...
fn test_comparison_subjunctive() {
    ...
    let _source = "ξ πέντε ἔστω. εἰ ξ πέντε μεῖζον ᾖ, «μείζον» λέγε.";
    compile_success(source);
}
...
fn test_print_string_split() {
    ...
    let _source = "ξ «α-β» ἔστω. ξ κατὰ «-» σχίζεται λέγε.";
    compile_success(source);
}
...
fn test_comparison_unknown_var() {
    ...
    let _source = "εἰ αγνωστον πέντε μεῖζον ᾖ, «ναι» λέγε.";
    compile_success(source);
}
...
fn test_struct_instantiation_fallthrough() {
    ...
    let _source = "ξ θετικά πέντε ἔστω.";
    compile_success(source);
}
...
fn test_print_with_operator_and_subject() {
    // Print "xi + 5"
    let _source = "ξ πέντε ἔστω. ξ καὶ 5 λέγε.";
    compile_success(source);
}
...
fn test_binding_unwrap() {
    // Bind unwrap
    let _source = "ξ τί 5 ἔστω. ψ ξ! ἔστω.";
    compile_success(source);
}
...
fn test_property_access_on_non_struct() {
    ...
    let _source = "ξ 5 ἔστω. που ξ λέγε.";
    compile_success(source);
}
...
fn test_struct_instantiation_unknown_type() {
    ...
    let _source = "ξ νέον ἄγνωστον 5 ἔστω.";
    compile_success(source);
}
...
fn test_print_subject_and_object() {
    ...
    let _source = "ξ 5 ἔστω. ψ 10 ἔστω. ξ ψ λέγε.";
    compile_success(source);
}
...
fn test_function_call_nominative_extra() {
    ...
    ξ ψ φ 5 ἔστω.
    ";
    // This might fail if "psi" is not defined, but here we just want to ensure it parses/analyzes
    // without crashing or weird errors.
    // If "psi" is undefined, it might be an error in binding logic, but the `detect_function_call`
    // should ideally pick up `phi` from nominatives.
    // However, if "psi" is undefined, `detect_variable_binding` might complain?
    // Wait, if it's a function call, it returns a Binding statement.
    // "xi" becomes the variable.
    // "phi(5)" becomes the value.
    // "yi" is ignored? Or treated as arg?
    // If `phi` is found in nominatives, `detect_function_call` takes `literals` as args.
    // "yi" is an object. It's not in literals.
    // So "yi" is effectively ignored or causes "unused" warning?
...
fn test_extract_value_object_literal_op() {
    ...
    let _source = "ψ 10 ἔστω. ξ ψ καὶ 2 ἔστω.";
    compile_success(source);
}
...
fn test_iterator_fallthrough() {
    ...
    let _source = "ξ 5 ἔστω. τοπικον λέγε.";
    compile_success(source);
}
