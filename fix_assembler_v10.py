        ...
        if let (Some(subject), Some(verb)) = (&self.state.subject, &self.state.verb) {
                ...
                && !crate::morphology::lexicon::is_find_verb(&verb.lemma)
            {
                ...
                for nom in &self.state.nominatives {
                    if nom.lemma == "ἄλλο" || nom.lemma == "μηδέν" || nom.lemma == "οὐδέν" || nom.lemma == "τι" || crate::morphology::lexicon::numeral_value(&nom.lemma).is_some() {
                        invalid = false;
                        break;
                    }
                }
                if invalid && (subject.lemma == "ἄλλο" || subject.lemma == "μηδέν" || subject.lemma == "οὐδέν" || subject.lemma == "τι" || crate::morphology::lexicon::numeral_value(&subject.lemma).is_some()) {
                    invalid = false;
                }
                if invalid {
                    return Err(AssemblyError::DoubleSubject);
