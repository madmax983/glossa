...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Some(Commands::Anatomist { input }) => {
            #[cfg(feature = "nova")]
            glossa::tools::anatomist::run_anatomist(&input)?;

            #[cfg(not(feature = "nova"))]
            {
                let _ = input;
                miette::bail!(
                    "The 'anatomist' command is experimental. Recompile glossa with '--features nova' to enable it."
                );
            }
        }
