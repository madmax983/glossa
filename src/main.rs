...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Some(Commands::Pygmalion { input }) => {
            #[cfg(feature = "nova")]
            glossa::tools::pygmalion::run_pygmalion(&input)?;

            #[cfg(not(feature = "nova"))]
            {
                let _ = input;
                miette::bail!(
                    "The 'pygmalion' command is experimental. Recompile glossa with '--features nova' to enable it."
                );
            }
        }
