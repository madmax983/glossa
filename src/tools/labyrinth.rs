...
impl CFGBuilder {
    ...
    fn build_simple_node(&mut self, label: &str, prev_node: &str) -> String {
        let id = self.add_node(label, "rect");
        self.add_edge(prev_node, &id, None);
        id
    }

    fn build_block_branch(
        &mut self,
        body: &[AnalyzedStatement],
        source_id: &str,
        end_id: &str,
        edge_label: Option<&str>,
        default_empty_target: Option<&str>,
    ) {
        let mut current_node = source_id.to_string();
        let mut has_nodes = false;

        for sub_stmt in body {
            let next_node = self.build_statement(sub_stmt, &current_node);
            if !has_nodes {
                // Fix the edge from condition to first statement of branch
                // by removing the previously added unconditional edge
                self.edges.pop();
                self.add_edge(source_id, &next_node, edge_label);
            }
        ...
        if has_nodes {
            self.add_edge(&current_node, end_id, None);
        } else if let Some(target) = default_empty_target {
            self.add_edge(source_id, target, edge_label);
        } else {
            self.add_edge(source_id, end_id, edge_label);
        }
    }
    ...
    fn build_if(
        ...
        // Then branch
        self.build_block_branch(then_body, &cond_id, &end_id, Some("Yes"), None);

        // Else branch
        if let Some(else_b) = else_body {
            self.build_block_branch(else_b, &cond_id, &end_id, Some("No"), None);
        } else {
    ...
    fn build_while(&mut self, body: &[AnalyzedStatement], prev_node: &str) -> String {
        let cond_id = self.add_node("While Condition", "diamond");
        self.add_edge(prev_node, &cond_id, None);
        let end_id = self.add_node("End While", "round");

        self.build_block_branch(body, &cond_id, &cond_id, Some("Yes"), Some(&cond_id));
        self.add_edge(&cond_id, &end_id, Some("No"));

        end_id
    }

    fn build_for(&mut self, variable: &str, body: &[AnalyzedStatement], prev_node: &str) -> String {
        let cond_id = self.add_node(&format!("For: {}", variable), "diamond");
        self.add_edge(prev_node, &cond_id, None);
        let end_id = self.add_node("End For", "round");

        self.build_block_branch(body, &cond_id, &cond_id, Some("Next"), Some(&cond_id));
    ...
    fn build_match(
        ...
        for (i, (_, body)) in arms.iter().enumerate() {
            ...
            self.build_block_branch(body, &match_id, &end_id, Some(&label), None);
        }
    ...
    fn build_statement(&mut self, stmt: &AnalyzedStatement, prev_node: &str) -> String {
        match stmt {
            AnalyzedStatement::Binding { name, .. } => {
                self.build_simple_node(&format!("Binding: {}", name), prev_node)
            }
            AnalyzedStatement::Assignment { name, .. } => {
                self.build_simple_node(&format!("Assignment: {}", name), prev_node)
            }
            AnalyzedStatement::Print(_) => self.build_simple_node("Print", prev_node),
            AnalyzedStatement::Expression(_) => self.build_simple_node("Expression", prev_node),
            AnalyzedStatement::Query(_) => self.build_simple_node("Query", prev_node),
            AnalyzedStatement::If {
                then_body,
                else_body,
                ..
            } => self.build_if(then_body, else_body, prev_node),
            AnalyzedStatement::While { body, .. } => self.build_while(body, prev_node),
            AnalyzedStatement::For { variable, body, .. } => {
                self.build_for(variable, body, prev_node)
            }
            AnalyzedStatement::Match { arms, .. } => self.build_match(arms, prev_node),
            AnalyzedStatement::Break => self.build_simple_node("Break", prev_node),
            AnalyzedStatement::Continue => self.build_simple_node("Continue", prev_node),
            AnalyzedStatement::Return { .. } => self.build_simple_node("Return", prev_node),
            AnalyzedStatement::FunctionDef { name, .. } => {
                self.build_simple_node(&format!("Function: {}", name), prev_node)
            }
            AnalyzedStatement::TypeDefinition { name, .. } => {
                self.build_simple_node(&format!("Type: {}", name), prev_node)
            }
            AnalyzedStatement::TraitDefinition { name, .. } => {
                self.build_simple_node(&format!("Trait: {}", name), prev_node)
            }
            AnalyzedStatement::TraitImplementation {
                ...
                self.build_simple_node(&format!("Impl {} for {}", trait_name, type_name), prev_node)
            }
            AnalyzedStatement::TestDeclaration { name, .. } => {
                self.build_simple_node(&format!("Test: {}", name), prev_node)
            }
        }
    }
