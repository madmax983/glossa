//! The Artisan (ὁ Τεχνίτης) - TypeScript Interface Generator
//!
//! This module implements the "Artisan" tool, which inspects the type definitions
//! (`εἶδος`) within a ΓΛΩΣΣΑ program and translates them into TypeScript
//! `interface` declarations.
//!
//! # Purpose
//!
//! Enables seamless full-stack integration where a ΓΛΩΣΣΑ backend can share type
//! definitions with a TypeScript frontend (e.g., React or Vue).
...
/// Runs the Artisan tool to generate TypeScript interfaces from Glossa types.
///
/// # Examples
///
/// ```rust,no_run
/// use glossa::tools::artisan::run_artisan;
/// use std::path::Path;
///
/// let input = Path::new("schema.γλ");
/// if let Err(e) = run_artisan(&input) {
...
pub fn run_artisan(input: &Path) -> Result<()> {
    ...
    let status = Status::start_with_symbol("Τεχνίτης (Generating TypeScript)", "🔨");
