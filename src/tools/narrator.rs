...
fn format_exprs_into(exprs: &[AnalyzedExpr], buf: &mut String) {
...
fn tell_expr_into(expr: &AnalyzedExpr, buf: &mut String) {
    match &expr.expr {
        ...
        AnalyzedExprKind::VerbCall { verb, args } => tell_verb_call(verb, args, buf),
        ...
        AnalyzedExprKind::ArrayLiteral(exprs) => tell_array_literal(exprs, buf),
        ...
        AnalyzedExprKind::FunctionCall { func, args } => tell_function_call(func, args, buf),
        AnalyzedExprKind::MethodCall {
            receiver,
            method,
            args,
        } => tell_method_call(receiver, method, args, buf),
        AnalyzedExprKind::StructInstantiation {
            type_name,
            fields,
            args,
        } => tell_struct_instantiation(type_name, fields, args, buf),
        AnalyzedExprKind::Lambda {
            params,
            body,
            capture_mode,
        } => tell_lambda(params, body, capture_mode, buf),
...
fn tell_verb_call(verb: &str, args: &[AnalyzedExpr], buf: &mut String) {
    let _ = write!(buf, "{}(", verb);
    format_exprs_into(args, buf);
    buf.push(')');
}

fn tell_array_literal(exprs: &[AnalyzedExpr], buf: &mut String) {
    buf.push('[');
    format_exprs_into(exprs, buf);
    buf.push(']');
}

fn tell_function_call(func: &str, args: &[AnalyzedExpr], buf: &mut String) {
