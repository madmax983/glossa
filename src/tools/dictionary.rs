...
pub fn lookup_word(word: &str) -> Result<()> {
    ...
    if let Some(entry) = lexicon::lookup(&normalized) {
        ...
        table.add_row(vec![
            Cell::new("Lemma"),
            Cell::new(entry.lemma).fg(Color::Green).add_attribute(Attribute::Bold),
        ]);
        ...
        if let Some(rust) = entry.rust_equiv {
            table.add_row(vec![
                ...
                Cell::new(rust).fg(Color::Red).add_attribute(Attribute::Bold),
            ]);
        }
    ...
    println!("   {}", "🔬 Morphological Analysis (All Possibilities)".bold().underlined());

    let mut table = Table::new();
    table.load_preset(presets::UTF8_FULL)
        .set_header(vec![
            Cell::new("Lemma").add_attribute(Attribute::Bold).fg(Color::Cyan),
    ...
    for analysis in analyses {
        ...
        if let Some(c) = analysis.case { grammar.push(format!("{}", c)); }
        if let Some(n) = analysis.number { grammar.push(format!("{}", n)); }
        if let Some(g) = analysis.gender { grammar.push(format!("{}", g)); }
        if let Some(p) = analysis.person { grammar.push(format!("{:?}", p)); }
        if let Some(t) = analysis.tense { grammar.push(format!("{:?}", t)); }
        if let Some(m) = analysis.mood { grammar.push(format!("{:?}", m)); }
        if let Some(v) = analysis.voice { grammar.push(format!("{:?}", v)); }
