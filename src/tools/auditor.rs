...
//! 2. A custom visitor function traverses every statement and expression in the AST.
...
pub fn run_auditor(input: &Path) -> Result<()> {
    ...
    for stmt in &program.statements {
        visit_statement(&mut usage_count, &mut mutation_count, &mut mutable_vars, stmt);
    }
...
fn visit_if_statement(
    ...
    visit_expr(usage_count, mutation_count, mutable_vars, condition);
    for s in then_body {
        visit_statement(usage_count, mutation_count, mutable_vars, s);
    }
...
fn visit_for_loop(
    ...
    visit_expr(usage_count, mutation_count, mutable_vars, iterator);
    for s in body {
        visit_statement(usage_count, mutation_count, mutable_vars, s);
    }
}
...
fn visit_match_statement(
    ...
    visit_expr(usage_count, mutation_count, mutable_vars, scrutinee);
    for (expr, stmts) in arms {
        visit_expr(usage_count, mutation_count, mutable_vars, expr);
        for s in stmts {
            visit_statement(usage_count, mutation_count, mutable_vars, s);
        }
    }
...
fn visit_statement(
    ...
    match stmt {
        AnalyzedStatement::Binding {
            ...
            visit_expr(usage_count, mutation_count, mutable_vars, value);
        }
        ...
        AnalyzedStatement::If {
            ...
            visit_if_statement(usage_count, mutation_count, mutable_vars, condition, then_body, else_body);
        }
        AnalyzedStatement::While { condition, body } => {
            visit_while_loop(usage_count, mutation_count, mutable_vars, condition, body);
        }
        AnalyzedStatement::For {
            ...
            visit_for_loop(usage_count, mutation_count, mutable_vars, variable, iterator, body);
        }
        AnalyzedStatement::Match { scrutinee, arms } => {
            visit_match_statement(usage_count, mutation_count, mutable_vars, scrutinee, arms);
        }
        AnalyzedStatement::FunctionDef { params, body, .. } => {
            visit_function_def(usage_count, mutation_count, mutable_vars, params, body);
        }
        AnalyzedStatement::Return { value } => {
            if let Some(v) = value {
                visit_expr(usage_count, mutation_count, mutable_vars, v);
            }
        }
...
fn visit_expr(
    ...
    match &expr.expr {
        ...
        AnalyzedExprKind::BinOp { left, right, .. } => {
            visit_expr(usage_count, mutation_count, mutable_vars, left);
            visit_expr(usage_count, mutation_count, mutable_vars, right);
        }
        AnalyzedExprKind::UnaryOp { operand, .. } => visit_expr(usage_count, mutation_count, mutable_vars, operand),
        AnalyzedExprKind::StructInstantiation { args, .. } => visit_exprs(usage_count, mutation_count, mutable_vars, args),
        AnalyzedExprKind::PropertyAccess { owner, .. } => visit_expr(usage_count, mutation_count, mutable_vars, owner),
        AnalyzedExprKind::MethodCall { receiver, args, .. } => {
            visit_expr(usage_count, mutation_count, mutable_vars, receiver);
            visit_exprs(usage_count, mutation_count, mutable_vars, args);
        }
        AnalyzedExprKind::FunctionCall { args, .. } => visit_exprs(usage_count, mutation_count, mutable_vars, args),
        AnalyzedExprKind::VerbCall { args, .. } => visit_exprs(usage_count, mutation_count, mutable_vars, args),
        AnalyzedExprKind::ArrayLiteral(exprs) => visit_exprs(usage_count, mutation_count, mutable_vars, exprs),
        AnalyzedExprKind::IndexAccess { array, index } => {
            visit_expr(usage_count, mutation_count, mutable_vars, array);
            visit_expr(usage_count, mutation_count, mutable_vars, index);
        }
        AnalyzedExprKind::Lambda { body, .. } => visit_expr(usage_count, mutation_count, mutable_vars, body),
        AnalyzedExprKind::Some(inner) => visit_expr(usage_count, mutation_count, mutable_vars, inner),
        AnalyzedExprKind::Ok(inner) => visit_expr(usage_count, mutation_count, mutable_vars, inner),
        AnalyzedExprKind::Err(inner) => visit_expr(usage_count, mutation_count, mutable_vars, inner),
        AnalyzedExprKind::Unwrap(inner) => visit_expr(usage_count, mutation_count, mutable_vars, inner),
        AnalyzedExprKind::Try(inner) => visit_expr(usage_count, mutation_count, mutable_vars, inner),
        AnalyzedExprKind::Assert { condition } => visit_expr(usage_count, mutation_count, mutable_vars, condition),
        AnalyzedExprKind::AssertEq { left, right } => {
            visit_expr(usage_count, mutation_count, mutable_vars, left);
            visit_expr(usage_count, mutation_count, mutable_vars, right);
        }
        AnalyzedExprKind::Range { start, end, .. } => {
            visit_expr(usage_count, mutation_count, mutable_vars, start);
            visit_expr(usage_count, mutation_count, mutable_vars, end);
        }
...
mod tests {
    ...
    fn test_auditor_visitor_coverage_expressions() {
        ...
        for kind in exprs {
            ...
            visit_expr(&mut usage_count, &mut mutation_count, &mut mutable_vars, &expr);
        }
    }
