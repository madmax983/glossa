...
pub fn visit_statement_auditor(
    ...
    match stmt {
        AnalyzedStatement::Binding {
            ...
            visit_expr_auditor(value, usage_count);
        }
        ...
        AnalyzedStatement::Print(exprs) => {
            for expr in exprs {
                visit_expr_auditor(expr, usage_count);
            }
        }
        ...
        AnalyzedStatement::If {
            ...
            visit_expr_auditor(condition, usage_count);
            for s in then_body {
                visit_statement_auditor(s, usage_count, mutation_count, mutable_vars);
            }
        ...
        AnalyzedStatement::For {
            ...
            visit_expr_auditor(iterator, usage_count);
            for s in body {
                visit_statement_auditor(s, usage_count, mutation_count, mutable_vars);
            }
        }
        AnalyzedStatement::Match { scrutinee, arms } => {
            visit_expr_auditor(scrutinee, usage_count);
        ...
        AnalyzedStatement::Return { value } => {
            if let Some(v) = value {
                visit_expr_auditor(v, usage_count);
            }
        }
...
pub fn visit_expr_auditor(expr: &AnalyzedExpr, usage_count: &mut FxHashMap<SmolStr, usize>) {
    match &expr.expr {
        ...
        AnalyzedExprKind::BinOp { left, right, .. } => {
            visit_expr_auditor(left, usage_count);
            visit_expr_auditor(right, usage_count);
        }
        AnalyzedExprKind::UnaryOp { operand, .. } => visit_expr_auditor(operand, usage_count),
        AnalyzedExprKind::StructInstantiation { args, .. } => {
            for arg in args {
                visit_expr_auditor(arg, usage_count);
            }
        }
        AnalyzedExprKind::PropertyAccess { owner, .. } => visit_expr_auditor(owner, usage_count),
        AnalyzedExprKind::MethodCall { receiver, args, .. } => {
            visit_expr_auditor(receiver, usage_count);
            for arg in args {
                visit_expr_auditor(arg, usage_count);
            }
        }
        ...
        AnalyzedExprKind::IndexAccess { array, index } => {
            visit_expr_auditor(array, usage_count);
            visit_expr_auditor(index, usage_count);
        }
        AnalyzedExprKind::Lambda { body, .. } => visit_expr_auditor(body, usage_count),
        AnalyzedExprKind::Some(inner) => visit_expr_auditor(inner, usage_count),
        AnalyzedExprKind::Ok(inner) => visit_expr_auditor(inner, usage_count),
        AnalyzedExprKind::Err(inner) => visit_expr_auditor(inner, usage_count),
        AnalyzedExprKind::Unwrap(inner) => visit_expr_auditor(inner, usage_count),
        AnalyzedExprKind::Try(inner) => visit_expr_auditor(inner, usage_count),
        AnalyzedExprKind::Assert { condition } => visit_expr_auditor(condition, usage_count),
        AnalyzedExprKind::AssertEq { left, right } => {
            visit_expr_auditor(left, usage_count);
            visit_expr_auditor(right, usage_count);
        }
        AnalyzedExprKind::Range { start, end, .. } => {
            visit_expr_auditor(start, usage_count);
            visit_expr_auditor(end, usage_count);
        }
...
mod tests {
    ...
    fn test_auditor_visitor_coverage_expressions() {
        ...
        for kind in exprs {
            ...
            visit_expr_auditor(&expr, &mut usage_count);
        }
    }
