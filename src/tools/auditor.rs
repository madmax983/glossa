...
fn visit_expr(expr: &AnalyzedExpr, usage_count: &mut FxHashMap<SmolStr, usize>) {
    match &expr.expr {
        ...
        | AnalyzedExprKind::Some(body)
        | AnalyzedExprKind::Ok(body)
        | AnalyzedExprKind::Err(body)
        | AnalyzedExprKind::Unwrap(body)
        | AnalyzedExprKind::Try(body)
        | AnalyzedExprKind::Assert { condition: body } => {
