...
/// Recursively visit expressions to gather variables usages
...
/// Recursively visit statements to gather variables usages and mutations
fn visit_statement(
    ...
    match stmt {
        ...
        AnalyzedStatement::Match { scrutinee, arms } => {
            ...
            for (pattern, stmts) in arms {
                visit_expr(pattern, usage_count);
                for s in stmts {
                    visit_statement(s, usage_count, mutation_count, mutable_vars);
                }
            }
