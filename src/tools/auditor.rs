...
//! 2. A custom visitor (`visit_statement` and `visit_expr`) traverses every statement and expression in the AST.
...
pub fn visit_expr(
    ...
    match &expr.expr {
        ...
        AnalyzedExprKind::Some(expr) => {
            visit_expr(expr, usage_count, mutation_count, mutable_vars);
        }
        AnalyzedExprKind::Ok(expr) => {
            visit_expr(expr, usage_count, mutation_count, mutable_vars);
        }
        AnalyzedExprKind::Err(expr) => {
            visit_expr(expr, usage_count, mutation_count, mutable_vars);
        }
        AnalyzedExprKind::Unwrap(expr) => {
            visit_expr(expr, usage_count, mutation_count, mutable_vars);
        }
        AnalyzedExprKind::Try(expr) => {
            visit_expr(expr, usage_count, mutation_count, mutable_vars);
        }
...
pub fn run_auditor(input: &Path) -> Result<()> {
    ...
    let status = Status::start_with_symbol("Λογιστής (Auditing Code)", "🕵️");
    ...
    for (var, count) in &mutation_count {
        if *count == 0 && mutable_vars.contains(var) && usage_count.get(var).unwrap_or(&0) > &0 {
            table.add_row(vec![
                ...
                Cell::new(var),
                Cell::new("Declared mutable (μετά) but never reassigned"),
            ]);
            issues += 1;
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_run_auditor_file_not_found() {
        let input_path = Path::new("nonexistent.gl");
