...
fn visit_statement(state: &mut AuditorState, stmt: &AnalyzedStatement) {
    match stmt {
...
fn visit_exprs(state: &mut AuditorState, exprs: &[AnalyzedExpr]) {
    for expr in exprs {
        visit_expr(state, expr);
    }
}

fn visit_expr(state: &mut AuditorState, expr: &AnalyzedExpr) {
    match &expr.expr {
