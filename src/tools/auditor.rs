...
pub fn audit_statement(
    ...
    match stmt {
        ...
        AnalyzedStatement::For { iterator, body, variable } => {
            audit_expr(iterator, usage_count);
            usage_count.insert(variable.clone(), 0);
            mutation_count.insert(variable.clone(), 0);
            for s in body {
                audit_statement(s, usage_count, mutation_count, mutable_vars);
            }
        }
        ...
        AnalyzedStatement::FunctionDef { params, body, .. } => {
            for (param_name, _) in params {
                ...
                mutation_count.insert(param_name.clone(), 0);
            }
            for s in body {
                audit_statement(s, usage_count, mutation_count, mutable_vars);
            }
...
pub fn audit_expr(
    ...
    // Avoid stack overflow on deeply nested expressions
    stacker::maybe_grow(32 * 1024, 1024 * 1024, || match &expr.expr {
        ...
        | AnalyzedExprKind::Some(operand)
        | AnalyzedExprKind::Ok(operand)
        | AnalyzedExprKind::Err(operand)
        | AnalyzedExprKind::Unwrap(operand)
        | AnalyzedExprKind::Try(operand) => {
            audit_expr(operand, usage_count);
        }
        AnalyzedExprKind::PropertyAccess { owner, .. } => {
            audit_expr(owner, usage_count);
        }
