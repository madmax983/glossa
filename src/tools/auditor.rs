...
mod tests {
    ...
    fn test_run_auditor_file_not_found() {
        let result = run_auditor(Path::new("does_not_exist.gl"));
        assert!(result.is_err());
    }

    #[test]
    fn test_run_auditor_syntax_error() {
        let dir = tempfile::tempdir().unwrap();
    ...
    fn test_run_auditor_success() {
        let dir = tempfile::tempdir().unwrap();
        let input_path = dir.path().join("success.gl");
        {
            let mut f = std::fs::File::create(&input_path).unwrap();
            f.write_all("«ok» λέγε.".as_bytes()).unwrap();
        }
