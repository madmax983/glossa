...
//! 2. The code traverses every statement and expression in the AST via pure functions.
...
fn check_statement(
    ...
    match stmt {
        AnalyzedStatement::Binding {
            ...
            check_expr(value, usage_count, mutation_count, mutable_vars);
        }
        ...
        | AnalyzedStatement::Query(exprs) => {
            for expr in exprs {
                check_expr(expr, usage_count, mutation_count, mutable_vars);
            }
        }
        AnalyzedStatement::If {
            ...
            check_expr(condition, usage_count, mutation_count, mutable_vars);
            for s in then_body {
                check_statement(s, usage_count, mutation_count, mutable_vars);
            }
        ...
        AnalyzedStatement::For {
            ...
            check_expr(iterator, usage_count, mutation_count, mutable_vars);
            for s in body {
                check_statement(s, usage_count, mutation_count, mutable_vars);
            }
        }
        AnalyzedStatement::Match { scrutinee, arms } => {
            check_expr(scrutinee, usage_count, mutation_count, mutable_vars);
        ...
        AnalyzedStatement::Return { value } => {
            if let Some(v) = value {
                check_expr(v, usage_count, mutation_count, mutable_vars);
            }
        }
...
fn check_expr(
    ...
    match &expr.expr {
        ...
        AnalyzedExprKind::BinOp { left, right, .. } => {
            check_expr(left, usage_count, mutation_count, mutable_vars);
            check_expr(right, usage_count, mutation_count, mutable_vars);
        }
        AnalyzedExprKind::UnaryOp { operand, .. } => {
            check_expr(operand, usage_count, mutation_count, mutable_vars)
        }
        ...
        | AnalyzedExprKind::ArrayLiteral(args) => {
            for arg in args {
                check_expr(arg, usage_count, mutation_count, mutable_vars);
            }
        }
        AnalyzedExprKind::PropertyAccess { owner, .. } => {
            check_expr(owner, usage_count, mutation_count, mutable_vars)
        }
        AnalyzedExprKind::MethodCall { receiver, args, .. } => {
            check_expr(receiver, usage_count, mutation_count, mutable_vars);
            for arg in args {
                check_expr(arg, usage_count, mutation_count, mutable_vars);
            }
        }
        AnalyzedExprKind::IndexAccess { array, index } => {
            check_expr(array, usage_count, mutation_count, mutable_vars);
            check_expr(index, usage_count, mutation_count, mutable_vars);
        }
        AnalyzedExprKind::Lambda { body, .. } => {
            check_expr(body, usage_count, mutation_count, mutable_vars)
        }
        ...
        | AnalyzedExprKind::Try(inner) => {
            check_expr(inner, usage_count, mutation_count, mutable_vars)
        }
        ...
        AnalyzedExprKind::Range { start, end, .. } => {
            check_expr(start, usage_count, mutation_count, mutable_vars);
            check_expr(end, usage_count, mutation_count, mutable_vars);
        }
