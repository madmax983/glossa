...
pub fn calculate_usage(
    ...
    match stmt {
        AnalyzedStatement::Binding {
            ...
            calculate_expr_usage(value, usage_count);
        }
        ...
        | AnalyzedStatement::Query(exprs) => {
            for expr in exprs {
                calculate_expr_usage(expr, usage_count);
            }
        }
        AnalyzedStatement::If {
            ...
            calculate_expr_usage(condition, usage_count);
            for s in then_body {
                calculate_usage(s, usage_count, mutation_count, mutable_vars);
            }
        ...
        AnalyzedStatement::For {
            ...
            calculate_expr_usage(iterator, usage_count);
            for s in body {
                calculate_usage(s, usage_count, mutation_count, mutable_vars);
            }
        }
        AnalyzedStatement::Match { scrutinee, arms } => {
            calculate_expr_usage(scrutinee, usage_count);
        ...
        AnalyzedStatement::Return { value } => {
            if let Some(v) = value {
                calculate_expr_usage(v, usage_count);
            }
        }
...
pub fn calculate_expr_usage(expr: &AnalyzedExpr, usage_count: &mut FxHashMap<SmolStr, usize>) {
    match &expr.expr {
        ...
        AnalyzedExprKind::BinOp { left, right, .. } => {
            calculate_expr_usage(left, usage_count);
            calculate_expr_usage(right, usage_count);
        }
        AnalyzedExprKind::UnaryOp { operand, .. } => calculate_expr_usage(operand, usage_count),
        AnalyzedExprKind::StructInstantiation { args, .. } => {
            for arg in args {
                calculate_expr_usage(arg, usage_count);
            }
        }
        AnalyzedExprKind::PropertyAccess { owner, .. } => calculate_expr_usage(owner, usage_count),
        AnalyzedExprKind::MethodCall { receiver, args, .. } => {
            calculate_expr_usage(receiver, usage_count);
            for arg in args {
                calculate_expr_usage(arg, usage_count);
            }
        }
        ...
        AnalyzedExprKind::IndexAccess { array, index } => {
            calculate_expr_usage(array, usage_count);
            calculate_expr_usage(index, usage_count);
        }
        AnalyzedExprKind::Lambda { body, .. } => calculate_expr_usage(body, usage_count),
        AnalyzedExprKind::Some(inner)
        | AnalyzedExprKind::Ok(inner)
        | AnalyzedExprKind::Err(inner)
        | AnalyzedExprKind::Unwrap(inner)
        | AnalyzedExprKind::Try(inner) => calculate_expr_usage(inner, usage_count),
        AnalyzedExprKind::Assert { condition } => calculate_expr_usage(condition, usage_count),
        AnalyzedExprKind::AssertEq { left, right } => {
            calculate_expr_usage(left, usage_count);
            calculate_expr_usage(right, usage_count);
        }
        AnalyzedExprKind::Range { start, end, .. } => {
            calculate_expr_usage(start, usage_count);
            calculate_expr_usage(end, usage_count);
        }
...
mod tests {
    ...
    fn test_auditor_visitor_coverage_statements() {
        let mut usage_count = rustc_hash::FxHashMap::default();
        let mut mutation_count = rustc_hash::FxHashMap::default();
        let mut mutable_vars = rustc_hash::FxHashSet::default();
    ...
    fn test_auditor_visitor_coverage_expressions() {
        ...
        for kind in exprs {
            ...
            calculate_expr_usage(&expr, &mut usage_count);
        }
    }
    ...
    fn test_auditor_error_paths() {
        ...
        let result = run_auditor(std::path::Path::new("nonexistent.gl"));
