...
fn visit_statement(next_id: &mut usize, output: &mut String, stmt: &AnalyzedStatement) -> usize {
    ...
    match stmt {
        ...
        AnalyzedStatement::Print(exprs) => visit_print_statement(next_id, output, id, exprs),
        AnalyzedStatement::Expression(exprs) => {
            visit_expression_statement(next_id, output, id, exprs)
        }
        AnalyzedStatement::Query(exprs) => visit_query_statement(next_id, output, id, exprs),
        ...
        AnalyzedStatement::Return { value } => {
            visit_return_statement(next_id, output, id, value.as_deref());
        }
...
fn visit_expr(next_id: &mut usize, output: &mut String, expr: &AnalyzedExpr) -> usize {
    ...
    match &expr.expr {
        AnalyzedExprKind::StringLiteral(s) => visit_string_literal_expr(output, id, s, &type_info),
        AnalyzedExprKind::NumberLiteral(n) => visit_number_literal_expr(output, id, *n, &type_info),
        AnalyzedExprKind::BooleanLiteral(b) => {
            visit_boolean_literal_expr(output, id, *b, &type_info)
        }
        ...
        AnalyzedExprKind::CollectionNew { collection_type } => {
            visit_collection_new_expr(output, id, collection_type, &type_info)
        }
...
fn visit_string_literal_expr(output: &mut String, id: usize, s: &str, type_info: &str) {
    emit_node(
        output,
        id,
        &format!("String\\n\\\"{}\\\"{}", s, type_info),
...
fn visit_number_literal_expr(output: &mut String, id: usize, n: i64, type_info: &str) {
    emit_node(
        output,
        id,
        &format!("Number\\n{}{}", n, type_info),
...
fn visit_boolean_literal_expr(output: &mut String, id: usize, b: bool, type_info: &str) {
    emit_node(
        output,
        id,
        &format!("Boolean\\n{}{}", b, type_info),
