...
pub fn run_haruspex(input: &Path) -> Result<()> {
    ...
    if let Err(e) = std::fs::write(&out_path, &dot) {
        return Err(miette::miette!("Failed to write DOT file: {}", e));
    }
...
fn print_dashboard(dot: &str, out_path: &Path) {
    ...
    println!(
        "   {}",
        format!("Raw DOT saved to: {}", out_path.display()).dim()
    );
    println!();
    println!(
        "   {}",
        "To view the graph, you can render it using Graphviz:".dim()
    );
    println!(
        "   {}",
        format!("dot -Tpng {} > ast.png", out_path.display()).dim()
    );
    println!();
}
...
mod tests {
    ...
    #[test]
    fn test_print_dashboard_output() {
        // Just verify it doesn't panic
        let fake_path = Path::new("dummy.dot");
        print_dashboard(
