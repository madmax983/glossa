...
fn visit_statement(next_id: &mut usize, output: &mut String, stmt: &AnalyzedStatement) -> usize {
    ...
    match stmt {
        AnalyzedStatement::Binding {
            ...
            visit_assignment_binding(next_id, output, id, "Binding", name, value, *mutable);
        }
        AnalyzedStatement::Assignment { name, value } => {
            visit_assignment_binding(next_id, output, id, "Assignment", name, value, false);
        }
        AnalyzedStatement::Print(exprs) => {
            visit_multi_expr_statement(next_id, output, id, "Print", exprs, "arg_")
        }
        AnalyzedStatement::Expression(exprs) => {
            visit_multi_expr_statement(next_id, output, id, "ExpressionStmt", exprs, "expr_")
        }
        AnalyzedStatement::Query(exprs) => {
            visit_multi_expr_statement(next_id, output, id, "Query", exprs, "arg_")
        }
...
fn visit_multi_expr_statement(
    ...
    emit_node(output, id, node_type, "lightgreen");
    for (i, expr) in exprs.iter().enumerate() {
        let child_id = visit_expr(next_id, output, expr);
        emit_edge(output, id, child_id, &format!("{}{}", prefix, i));
    }
}
...
fn visit_expr(next_id: &mut usize, output: &mut String, expr: &AnalyzedExpr) -> usize {
    ...
    match &expr.expr {
        AnalyzedExprKind::StringLiteral(s) => {
            visit_literal_expr(output, id, "String", s, &type_info, true)
        }
        AnalyzedExprKind::NumberLiteral(n) => {
            visit_literal_expr(output, id, "Number", n, &type_info, false)
        }
        AnalyzedExprKind::BooleanLiteral(b) => {
            visit_literal_expr(output, id, "Boolean", b, &type_info, false)
        }
        ...
        AnalyzedExprKind::Some(e) => {
            visit_wrapper_expr(next_id, output, id, "Some", e, "value", &type_info)
        }
        AnalyzedExprKind::None => {
            emit_node(output, id, &format!("None{}", type_info), "lightyellow")
        }
        AnalyzedExprKind::Ok(e) => {
            visit_wrapper_expr(next_id, output, id, "Ok", e, "value", &type_info)
        }
        AnalyzedExprKind::Err(e) => {
            visit_wrapper_expr(next_id, output, id, "Err", e, "error", &type_info)
        }
        AnalyzedExprKind::Unwrap(e) => {
            visit_wrapper_expr(next_id, output, id, "Unwrap", e, "target", &type_info)
        }
        AnalyzedExprKind::Try(e) => {
            visit_wrapper_expr(next_id, output, id, "Try", e, "target", &type_info)
        }
...
fn visit_wrapper_expr(
    ...
    emit_node(output, id, &format!("{}{}", name, type_info), "lightyellow");
