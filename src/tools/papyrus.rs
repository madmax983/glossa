...
pub fn run_papyrus(input: &Path) -> Result<()> {
    ...
    for stmt in &program.statements {
        if let AnalyzedStatement::TypeDefinition { name, fields } = stmt {
            ...
            table_summaries.push((name.clone(), fields.len()));
        }
    }

    let out_path = input.with_extension("sql");
    if let Err(e) = std::fs::write(&out_path, output) {
        return Err(miette::miette!("Failed to write SQL file: {}", e));
    }
    ...
    for (name, cols) in table_summaries {
        table.add_row(vec![Cell::new(name), Cell::new(cols.to_string())]);
    }

    if table.row_iter().count() == 0 {
        table.add_row(vec![
            Cell::new("No schemas found").fg(Color::DarkGrey),
    ...
    println!(
        "   {}",
        format!("Raw SQL saved to: {}", out_path.display()).dim()
    );
    println!();
