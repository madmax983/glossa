//! The Envoy (ὁ Πρεσβευτής) - TypeScript Interface Exporter
//!
...
//! # Purpose
//!
//! The Envoy bridges the Ancient Greek backend with modern web frontends by
//! generating TypeScript definitions for Glossa types, allowing them to be shared.
...
/// Runs the Envoy tool to generate TypeScript interfaces from Glossa types.
pub fn run_envoy(input: &Path) -> Result<()> {
    if !input.exists() {
        return Err(miette::miette!("Ἀρχεῖον οὐχ εὑρέθη: {}", input.display()));
    }

    let status = Status::start_with_symbol("Πρεσβευτής (Generating TypeScript Interfaces)", "📜");
    ...
    for stmt in &program.statements {
        if let AnalyzedStatement::TypeDefinition { name, fields } = stmt {
            ...
            for (field_name, field_type) in fields {
                ...
                let optional = if let GlossaType::Option(_) = field_type {
                    "?"
                } else {
                    ""
                };
