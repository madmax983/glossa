...
/// Calculates the maximum loop depth of a statement.
#[allow(clippy::only_used_in_recursion)]
fn get_max_depth(stmt: &AnalyzedStatement, current_depth: usize) -> usize {
    match stmt {
        AnalyzedStatement::While { body, .. } | AnalyzedStatement::For { body, .. } => {
            let next_depth = current_depth + 1;
            let mut max_child = next_depth;
            for s in body {
                max_child = max_child.max(get_max_depth(s, next_depth));
            }
            max_child
        }
        AnalyzedStatement::If {
            ...
            let mut max_child = current_depth;
            for s in then_body {
                max_child = max_child.max(get_max_depth(s, current_depth));
            }
...
/// using a recursive function. The result is presented to the user in a
...
pub fn run_gnomon(input: &Path) -> Result<()> {
    ...
    for stmt in &program.statements {
        max_depth = max_depth.max(get_max_depth(stmt, 0));
    }
...
mod tests {
    ...
    fn test_gnomon_while_loop() {
        ...
        assert_eq!(get_max_depth(&stmt, 0), 1);
    }
    ...
    fn test_gnomon_nested_loops() {
        ...
        assert_eq!(get_max_depth(&outer_loop, 0), 2);
    }
}
