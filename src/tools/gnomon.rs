...
fn visit_statement_for_depth(
    ...
    match stmt {
        AnalyzedStatement::While { body, .. } => {
            ...
            for s in body {
                visit_statement_for_depth(s, next_depth, max_depth);
            }
        }
        ...
        AnalyzedStatement::If {
            ...
            for s in then_body {
                visit_statement_for_depth(s, current_depth, max_depth);
            }
...
pub fn run_gnomon(input: &Path) -> Result<()> {
    ...
    for stmt in &program.statements {
        visit_statement_for_depth(stmt, 0, &mut max_depth);
    }
...
mod tests {
    ...
    fn test_gnomon_nested_loops() {
        ...
        visit_statement_for_depth(&outer_loop, 0, &mut max_depth);
