...
fn calculate_max_depth(stmt: &AnalyzedStatement, current_depth: usize) -> usize {
    ...
    match stmt {
        AnalyzedStatement::While { body, .. } | AnalyzedStatement::For { body, .. } => {
            ...
            for s in body {
                let d = calculate_max_depth(s, next_depth);
                if d > max_depth {
                    max_depth = d;
                }
            }
        ...
        AnalyzedStatement::If {
            ...
            for s in then_body {
                let d = calculate_max_depth(s, current_depth);
                if d > max_depth {
                    max_depth = d;
                }
            }
...
pub fn run_gnomon(input: &Path) -> Result<()> {
    ...
    for stmt in &program.statements {
        let d = calculate_max_depth(stmt, 0);
        if d > max_depth {
            max_depth = d;
        }
    }
...
mod tests {
    ...
    fn test_gnomon_while_loop() {
        ...
        let max_depth = calculate_max_depth(&stmt, 0);
    ...
    fn test_gnomon_nested_loops() {
        ...
        let max_depth = calculate_max_depth(&outer_loop, 0);
