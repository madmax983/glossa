...
/// Recursively calculates the maximum loop depth for a slice of statements.
///
...
pub fn calculate_max_depth(stmts: &[AnalyzedStatement]) -> usize {
    let mut max = 0;
    for stmt in stmts {
...
mod tests {
    ...
    fn test_gnomon_while_loop() {
        ...
        let depth = calculate_max_depth(&[stmt]);
        assert_eq!(depth, 1);
    }
