...
/// using a procedural recursive traversal. The result is presented to the user in a
    ...
    match stmt {
        AnalyzedStatement::While { body, .. } => {
            *current_depth += 1;
            ...
            for s in body {
                visit_statement(current_depth, max_depth, s);
            }
            *current_depth -= 1;
        }
...
pub fn run_gnomon(input: &Path) -> Result<()> {
    ...
    for stmt in &program.statements {
        visit_statement(&mut current_depth, &mut max_depth, stmt);
    }
...
mod tests {
    ...
    fn test_gnomon_nested_loops() {
        ...
        visit_statement(&mut current_depth, &mut max_depth, &outer_loop);
