...
/// Recursively calculates loop depth metrics.
///
...
/// using the [`calculate_max_depth`]. The result is presented to the user in a
...
mod tests {
    ...
    #[test]
    fn test_gnomon_deeply_nested_loops() {
        let mut max_depth = 0;
        let loop1 = AnalyzedStatement::For {
            variable: SmolStr::new("a"),
            iterator: dummy_expr(),
            body: vec![],
        };
        let loop2 = AnalyzedStatement::While {
            condition: dummy_expr(),
            body: vec![loop1],
        };
        let loop3 = AnalyzedStatement::For {
            variable: SmolStr::new("b"),
            iterator: dummy_expr(),
            body: vec![loop2],
        };
        calculate_max_depth(&loop3, 0, &mut max_depth);
