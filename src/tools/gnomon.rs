    ...
    match stmt {
        AnalyzedStatement::While { body, .. } => {
            *current_depth += 1;
            ...
            for s in body {
                visit_statement_gnomon(s, current_depth, max_depth);
            }
            *current_depth -= 1;
        }
...
/// using the `visit_statement_gnomon`. The result is presented to the user in a
...
pub fn run_gnomon(input: &Path) -> Result<()> {
    ...
    for stmt in &program.statements {
        visit_statement_gnomon(stmt, &mut current_depth, &mut max_depth);
    }
...
mod tests {
    ...
    fn test_gnomon_nested_loops() {
        ...
        visit_statement_gnomon(&outer_loop, &mut current_depth, &mut max_depth);
