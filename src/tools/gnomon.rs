...
pub fn check_statement(stmt: &AnalyzedStatement, current_depth: &mut usize, max_depth: &mut usize) {
    match stmt {
        AnalyzedStatement::While { body, .. } | AnalyzedStatement::For { body, .. } => {
            *current_depth += 1;
            ...
            for s in body {
                check_statement(s, current_depth, max_depth);
            }
            *current_depth -= 1;
        }
...
/// using pure functions. The result is presented to the user in a
...
pub fn run_gnomon(input: &Path) -> Result<()> {
    ...
    for stmt in &program.statements {
        check_statement(stmt, &mut current_depth, &mut max_depth);
    }
...
mod tests {
    ...
    fn test_gnomon_nested_loops() {
        ...
        check_statement(&outer_loop, &mut current_depth, &mut max_depth);
