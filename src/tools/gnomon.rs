...
pub fn calculate_loop_depth(stmt: &AnalyzedStatement, current_depth: &mut usize, max_depth: &mut usize) {
    match stmt {
        AnalyzedStatement::While { body, .. } => {
            *current_depth += 1;
            ...
            for s in body {
                calculate_loop_depth(s, current_depth, max_depth);
            }
            *current_depth -= 1;
        }
...
/// using `calculate_loop_depth`. The result is presented to the user in a
...
pub fn run_gnomon(input: &Path) -> Result<()> {
    ...
    for stmt in &program.statements {
        calculate_loop_depth(stmt, &mut current_depth, &mut max_depth);
    }
...
mod tests {
    ...
    fn test_gnomon_nested_loops() {
        ...
        calculate_loop_depth(&outer_loop, &mut current_depth, &mut max_depth);
