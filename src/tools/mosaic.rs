...
pub fn run_mosaic_on_source<W: Write>(source: &str, writer: &mut W) -> Result<()> {
    ...
    writeln!(
        writer,
        "   {}",
        "Semantic Assembly Visualizer".italic().dim()
    )
    .into_diagnostic()?;
    writeln!(writer).into_diagnostic()?;
    ...
    for (i, stmt) in program.statements.iter().enumerate() {
        writeln!(
            writer,
            "{}",
            format!("Statement #{}", i + 1).yellow().bold()
        )
        .into_diagnostic()?;

        match stmt {
            Statement::Regular { .. } => match assemble_statement(stmt) {
            ...
            Statement::TraitImpl(def) => {
                writeln!(
                    ...
                    def.trait_name.original.as_str().cyan(),
                    def.type_name.original.as_str().cyan()
                )
                .into_diagnostic()?;
            }
            Statement::TestDeclaration(def) => {
                writeln!(writer, "  Test Declaration: {}", def.name.as_str().cyan())
                    .into_diagnostic()?;
            }
        }
        writeln!(writer).into_diagnostic()?;
...
fn print_assembled_table<W: Write>(stmt: &AssembledStatement, writer: &mut W) -> Result<()> {
    ...
    if let Some(verb) = &stmt.verb {
        table.add_row(vec![
            ...
            Cell::new(format!(
                "{:?} {:?} {:?}",
                verb.person, verb.number, verb.tense
            )),
        ]);
    }
