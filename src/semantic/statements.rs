...
mod tests {
    ...
    fn test_check_recursive_type_through_result() {
        ...
        assert!(check_recursive_type("A", &result_err, &mut visited2));
    }
#[cfg(test)]
mod tests {
    use super::*;
    ...
    #[test]
    fn test_check_recursive_type_scalars() {
        // Scalar types (Number, String, Boolean, Unit) are never recursive
        let types = vec![
            GlossaType::Number,
            GlossaType::String,
            GlossaType::Boolean,
        ...
        for ty in types {
            let mut visited = HashSet::new();
            assert!(!check_recursive_type("A", &ty, &mut visited));
        }
    }

    #[test]
    fn test_check_recursive_type_diamond_dependency() {
        // A depends on B and C.
        // B depends on D.
        // C depends on D.
        // D depends on nothing (safe).
        // This forms a diamond. We want to ensure we visit D via both paths safely.
        // Also checks the `visited` logic.

        let type_d = GlossaType::Struct {
            name: "D".into(),
            gender: crate::morphology::Gender::Neuter,
            fields: vec![],
        };
    ...
    #[test]
    fn test_check_recursive_type_cycle_not_involving_target() {
        // Checking A.
        // A -> B
        // B -> C
        // C -> B (cycle B-C-B)
        // A is NOT recursive (doesn't contain A). But the traversal encounters a cycle.
        // The `visited` set should handle this without infinite loop.

        // We need to construct this carefully since GlossaType::Struct holds data by value.
        // We simulate the graph traversal behavior.

        // C contains B
        let type_b_ref = GlossaType::Struct {
            name: "B".into(),
            gender: crate::morphology::Gender::Neuter,
            fields: vec![], // In reality this would recurse, but for the check we stop at names.
        };

        let type_c = GlossaType::Struct {
            name: "C".into(),
            gender: crate::morphology::Gender::Neuter,
            fields: vec![("b".into(), type_b_ref.clone())],
        };
