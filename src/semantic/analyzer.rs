...
//! It breaks circular dependencies between control flow/declarations
...
pub fn analyze(stmt: &Statement, scope: &mut Scope) -> Result<Vec<AnalyzedStatement>, GlossaError> {
    ...
    if let Some(block_stmts) = extract_block_statements(stmt) {
        ...
        for s in block_stmts {
            analyzed.extend(analyze(s, &mut block_scope)?);
        }
        return Ok(analyzed);
    }
...
pub fn analyze_program(program: &Program) -> Result<AnalyzedProgram, GlossaError> {
    ...
    for stmt in &program.statements {
        ...
        analyzed_statements.extend(analyze(stmt, &mut scope)?);
    }
