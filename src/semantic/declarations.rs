            ...
            let body = if let Some(body_stmts) = &method.body {
                ...
                // Create a child scope for the method
                {
                    ...
                    for body_stmt in body_stmts {
                        analyzed_body
                            .extend(crate::semantic::analyzer::analyze(body_stmt, &mut scope)?);
                    }
                }
                Some(analyzed_body)
            } else {
    ...
    for method in &trait_impl.methods {
        ...
        let mut analyzed_body = Vec::new();
        {
            ...
            for body_stmt in &method.body {
                analyzed_body.extend(crate::semantic::analyzer::analyze(body_stmt, &mut scope)?);
            }
        }
...
pub fn parse_function_definition(
    ...
    let mut body_statements = Vec::new();
    {
        ...
        for expr in body_exprs {
            ...
            body_statements.extend(crate::semantic::analyzer::analyze(
                &clause_stmt,
                &mut function_scope,
            )?);
        }
...
pub fn analyze_test_declaration(
    ...
    // Create a child scope for the test
    {
        ...
        for body_stmt in &test_decl.body {
            analyzed_body.extend(crate::semantic::analyzer::analyze(
                body_stmt,
                &mut test_scope,
            )?);
        }
