...
pub fn classify_assembled_statement(
    ...
    if let Some(result) = classify_iterator_pattern_wrapper(asm_stmt, scope)? {
        return Ok(result);
    }

    if let Some(result) = classify_property_access_print(asm_stmt, scope)? {
        return Ok(result);
    }
    ...
    if let Some(result) = classify_pop(asm_stmt, scope)? {
        return Ok(result);
    }

    if let Some(result) = classify_push(asm_stmt, scope)? {
        return Ok(result);
    }

    if let Some(result) = classify_insert(asm_stmt, scope)? {
        return Ok(result);
    }
...
fn classify_iterator_pattern_wrapper(
    asm_stmt: &AssembledStatement,
    scope: &mut Scope,
