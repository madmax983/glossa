...
pub fn classify_assembled_statement(
    ...
    if let Some(result) = classify_iterator_pattern(asm_stmt, scope)? {
        return Ok(result);
    }

    if let Some(result) = classify_property_access(asm_stmt, scope)? {
        return Ok(result);
    }

    if let Some(result) = classify_struct_instantiation(asm_stmt, scope)? {
        return Ok(result);
    }

    if let Some(result) = classify_function_call(asm_stmt, scope)? {
        return Ok(result);
    }

    if let Some(result) = classify_comparison(asm_stmt, scope)? {
        return Ok(result);
    }

    if let Some(result) = classify_binding(asm_stmt, scope)? {
        return Ok(result);
    }

    if let Some(result) = classify_assignment(asm_stmt, scope)? {
        return Ok(result);
    }

    if let Some(result) = classify_collection_op(asm_stmt, scope)? {
        return Ok(result);
    }

    if let Some(result) = classify_print(asm_stmt, scope)? {
        return Ok(result);
    }

    if let Some(result) = classify_query(asm_stmt, scope)? {
        return Ok(result);
    }
...
mod tests {
    ...
    fn create_find_verb() -> VerbConstituent {
        VerbConstituent {
            lemma: "ευρισκω".into(),
            original: "εὑρέ".to_string(),
            person: Some(Person::Second),
            number: Some(Number::Singular),
    ...
    #[test]
    fn test_iterator_pattern_classification() {
        let mut scope = Scope::new();
        let mut asm_stmt = default_assembled_statement();
        // ξ τριῶν μείζον εὑρέ
        asm_stmt.verb = Some(create_find_verb());
        // Subject: iterable variable
        asm_stmt.subject = Some(Constituent {
            lemma: "ξ".to_string(),
            original: "ξ".to_string(),
            case: Case::Nominative,
            number: Some(Number::Singular),
            gender: None,
        });
        ...
        asm_stmt.adjectives = vec![Constituent {
            lemma: "μειζον".to_string(),
            original: "μεῖζον".to_string(),
            case: Case::Nominative,
            number: Some(Number::Singular),
        ...
        assert!(result.is_ok());
        // We mainly check it runs and returns Ok.
        // It might return None if detect_iterator_pattern fails matching (it has strict conditions),
        // but it shouldn't panic or error out with a GlossaError for valid input format.
    }

    #[test]
    fn test_struct_instantiation_classification() {
        let mut scope = Scope::new();
        let mut asm_stmt = default_assembled_statement();
        ...
        let result = classify_struct_instantiation(&asm_stmt, &mut scope);
        assert!(result.is_ok());
        let classification = result.unwrap();
        assert!(classification.is_some());
        let (kind, _) = classification.unwrap();
    ...
    fn test_comparison_classification() {
        ...
        let mut verb = create_binding_verb();
        verb.mood = Some(Mood::Subjunctive); // Required for comparison
        asm_stmt.verb = Some(verb);
        ...
        scope.define("α".to_string(), GlossaType::Number);

        let result = classify_comparison(&asm_stmt, &mut scope);
        assert!(result.is_ok());
        let classification = result.unwrap();
        assert!(classification.is_some());
        let (kind, _) = classification.unwrap();
    ...
    fn test_query_classification() {
        ...
        assert!(classification.is_some());
        let (kind, _) = classification.unwrap();
        assert!(matches!(kind, StatementKind::Query));
    }
}
