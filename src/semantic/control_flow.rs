...
fn skip_first_word_and_parse(
    ...
    // We bypass the top-level MissingVerb check by temporarily treating the statement
    // as a query, which is exempt from verb requirements.
    let stmt = Statement::Regular {
    ...
    match converted {
        AnalyzedStatement::Query(mut exprs) | AnalyzedStatement::Expression(mut exprs) => {
            if !exprs.is_empty() {
                Ok(exprs.remove(0))
            } else {
