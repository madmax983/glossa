...
fn parse_while_loop(
    ...
    let body_analyzed = crate::semantic::analyzer::analyze(&body_stmt, scope)?;
...
fn parse_for_range_loop(
    ...
    let body_analyzed = {
        ...
        crate::semantic::analyzer::analyze(&body_stmt, &mut loop_scope)?
    };
...
fn parse_conditional(
    ...
    let then_body = if then_clause.expressions.is_empty() {
        ...
        crate::semantic::analyzer::analyze(&stmt, scope)?
    };
    ...
    let else_body = if then_clause.expressions.len() > 1 && stmt.clauses().len() >= 3 {
        ...
        if check_else_pattern_in_expression(second_expr) {
            ...
            Some(crate::semantic::analyzer::analyze(&stmt, scope)?)
        }
