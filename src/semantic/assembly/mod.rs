...
impl Assembler {
    ...
    fn check_missing_verb(&self, ctx: &StatementContext) -> Result<(), AssemblyError> {
        ...
        {
            // Only skip missing verb for match arms in valid contexts, but standalone variable is not a valid statement
            if self.state.literals.is_empty() && self.state.operators.is_empty() && self.state.unwraps.is_empty() && self.state.blocks.is_empty() && self.state.nested_phrases.is_empty() && self.state.arrays.is_empty() && !crate::morphology::lexicon::numeral_value(&subject.lemma).is_some() {
                // Not a numeral, no other tokens, just a naked subject. Treat as missing verb.
