    ...
    pub fn finalize(&mut self) -> Result<AssembledStatement, AssemblyError> {
            ...
            && self.state.operators.is_empty()
        {
            let is_binding = self
                ...
                .verb
                .as_ref()
                .map(|v| crate::morphology::lexicon::is_binding_verb(&v.lemma))
                .unwrap_or(false);
            let is_find = self
                .state
                .verb
                .as_ref()
                .map(|v| crate::morphology::lexicon::is_find_verb(&v.lemma))
                .unwrap_or(false);
            let is_function_call = !self.state.nested_phrases.is_empty()
                || !self.state.blocks.is_empty()
                || !self.state.literals.is_empty();
            let is_special_pattern = !self.state.property_accesses.is_empty()
                || self.state.is_query
                || !self.state.adjectives.is_empty()
                || !self.state.genitives.is_empty();

            if !is_binding && !is_find && !is_function_call && !is_special_pattern {
